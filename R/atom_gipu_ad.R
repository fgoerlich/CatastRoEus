#' ATOM INSPIRE: Reference database for ATOM Addresses in Gipuzkoa
#'
#' @description
...
#'
#' @source
#' [INSPIREGIPUZCOA](https://b5m.gipuzkoa.eus/web5000/es/servicios-inspire)
#'
#' @family ATOM
#' @family addresses
#'
...
#'
#' @rdname catreus_gipu_atom_get_addresses_db
#' @export
#'
#' @return
...
#' @examples
#' \donttest{
#' catreus_gipu_atom_get_addresses_db_all()
#' }

catreus_gipu_atom_get_addresses_db_all <- function(cache = TRUE,
                                                   update_cache = FALSE,
                                                   cache_dir = NULL,
                                                   verbose = FALSE) {
  api_entry <- paste0(
    "https://b5m.gipuzkoa.eus/",
    "inspire/download/",
    "addresses.xml"
  )
...
#' ATOM INSPIRE: Download all the addresses of a municipality in Gipuzkoa
#' 
...
#'
#' @inheritParams catreus_gipu_atom_get_addresses_db
#' @param munic Municipality to extract, It can be a part of a string or the
#'   addresses code. See [catreus_gipu_atom_get_addresses_db].
#' @examples
#' \donttest{
#'
#' s <- catreus_gipu_atom_get_addresses("ALKIZA")
#'
#' library(ggplot2)
#'
#' ggplot(s) +
...
catreus_gipu_atom_get_addresses <- function(munic,
                                            cache = TRUE,
                                            update_cache = FALSE,
                                            cache_dir = NULL,
                                            verbose = FALSE) {
  all <- catreus_gipu_atom_get_addresses_db_all(
    cache = cache,
